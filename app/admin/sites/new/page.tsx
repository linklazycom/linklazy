...
export default function AdminNewSitePage() {
  ...
  const [owner, setOwner] = useState<AccountOption | null>(null);
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!owner) {
      setError("Pick an account to assign this site to.");
      return;
    }
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="mb-1 font-display text-2xl font-medium">List a site (admin)</h1>
      <p className="mb-6 text-sm text-muted">
        This skips the seller submission flow — the site goes live as approved and verified
        immediately, assigned to whichever account you pick below.
      </p>

      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="rounded-chip border border-line bg-white p-5">
          <h2 className="mb-4 text-sm font-medium">Assign to</h2>
          <AccountPicker selected={owner} onSelect={setOwner} />
        </div>
        ...
        <Button type="submit" disabled={loading} className="w-full">
          {loading ? "Listing site…" : "List & assign site"}
        </Button>
      </form>
    </div>
  );
